...
export default function StudentPage() {
  ...
  return (
    <>
      <Helmet>
        <title>{`./kma_score --name=${studentStatistics.name}`}</title>
        <meta
          property="og:title"
          content={`./kma_score --name=${studentStatistics.name}`}
        />
        <meta
          name="description"
          content={`Điểm số của ${studentStatistics.name}`}
        />
        <meta
          name="og:description"
          content={`Điểm số của ${studentStatistics.name}`}
      ...
      <div className="md:flex overflow-auto bg-black px-5 md:px-24 lg:px-36 pt-16 md:pt-28 pb-5">
        <div className="space-y-2 w-full">
          ...
          <Alert type="warning">
            Do điểm các môn tiếng Anh cơ bản có thể quy đổi từ các chứng chỉ,
            điểm được hiển thị có thể không chính xác
          </Alert>
